// Ported from: http://webstaff.itn.liu.se/~stegu/simplexnoise/simplexnoise.pdf
...
namespace NoiseTest
{
    public class SimplexNoise : IFunction2D, IFunction3D, IFunction4D
    {
        ...
        private static readonly double _F2 = 0.5 * (Math.Sqrt(3.0) - 1.0);
        private static readonly double _G2 = (3.0 - Math.Sqrt(3.0)) / 6.0;

        private static readonly double _F3 = 1.0 / 3.0;
        private static readonly double _G3 = 1.0 / 6.0;

        private static readonly double _F4 = (Math.Sqrt(5.0) - 1.0) / 4.0;
        private static readonly double _G4 = (5.0 - Math.Sqrt(5.0)) / 20.0;
        ...
        public SimplexNoise(Random rng)
        {
            var allValues = new List<byte>();
            for (int i = 0; i < 256; i++)
            {
                allValues.Add((byte)i);
            }
            var data = new byte[256];
            var extraData = new byte[8];
            for (int i = 0; i < 256; i++)
            {
                var index = rng.Next(allValues.Count);
                data[i] = allValues[index];
                allValues.RemoveAt(index);
            }
            rng.NextBytes(extraData);
            // Random offsets to prevent same coordinates (i.e. [0, 0] [1, 1] [2, 2]) from returning 0
            _dx = rng.NextDouble() * 1000.0;
            _dy = rng.NextDouble() * 1000.0;
            _dz = rng.NextDouble() * 1000.0;
            _dw = rng.NextDouble() * 1000.0;
            _perm = data.Concat(data).ToArray();
        }
        ...
        public double Evaluate(double x, double y)
        {
            x += _dx;
            y += _dy;
            double n0 = 0.0, n1 = 0.0, n2 = 0.0; // Noise contributions from the three corners
        ...
        public double Evaluate(double x, double y, double z)
        {
            ...
            y += _dy;
            z += _dz;
            double n0 = 0.0, n1 = 0.0, n2 = 0.0, n3 = 0.0; // Noise contributions from the four corners
            // Skew the input space to determine which simplex cell we're in
        ...
        public double Evaluate(double x, double y, double z, double w)
        {
            ...
            double n0 = 0.0, n1 = 0.0, n2 = 0.0, n3 = 0.0, n4 = 0.0; // Noise contributions from the five corners
